...
def main(params):
    ...
    """
    # setup paths
    rows, cols = params["SUB_ROW"], params["SUB_COL"]
    _merge_linrate(rows, cols, params)
    if params[pyrate.constants.TIME_SERIES_CAL]:
...
def _merge_linrate(rows, cols, params):
    ...
    if "tif" in base_unw_paths[0].split(".")[1]:
        dest_tifs = pyrate.configuration.get_dest_paths(base_unw_paths, crop, params, xlks)
    ...
    tiles = pyrate.process.get_tiles(dest_tifs[0], rows, cols)

    # linrate aggregation
    [_save_linrate(ifgs, params, tiles, out_type=t) for t in ['linrate', 'linerror', 'linsamples']]
...
def _save_linrate(ifgs_dict, params, tiles, out_type):
    ...
    dest = os.path.join(params[pyrate.constants.OUT_DIR], out_type + ".tif")
    ...
    for t in tiles:
        rate_file = os.path.join(params[pyrate.constants.TMPDIR], out_type + '_{}.npy'.format(t.index))
    ...
    npy_rate_file = os.path.join(params[pyrate.constants.OUT_DIR], out_type + '.npy')
...
def _merge_timeseries(rows, cols, params):
    ...
    # to all processes.
    process_tifs = range(2 * no_ts_tifs)
    ...
    log.info('process {} will write {} ts (incr/cuml) tifs of ''total {}'.format(1, len(process_tifs), no_ts_tifs * 2))
    for i in process_tifs:
        ...
        if i < no_ts_tifs:
            ...
            dest = os.path.join(params[pyrate.constants.OUT_DIR],'tscuml' + "_" + str(epochlist.dates[i + 1]) + ".tif")
            ...
            # sequence position; first time slice is #0
            md['SEQUENCE_POSITION'] = i+1
            dest = os.path.join(params[pyrate.constants.OUT_DIR],'tsincr' + "_" + str(epochlist.dates[i + 1]) + ".tif")
            md[ifc.DATA_TYPE] = ifc.INCR
            shared.write_output_geotiff(md, gt, wkt, tsincr_g, dest, np.nan)
    log.info('process {} finished writing {} ts (incr/cuml) tifs of total {}'.format(1, len(process_tifs), no_ts_tifs * 2))
